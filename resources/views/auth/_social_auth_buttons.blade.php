...
@if ($enabledProviders !== [])
    <div class="auth-social-buttons d-flex gap-2 justify-content-center flex-wrap {{ $class ?? '' }}">
        @if ($googleSignInEnabled ?? false)
            <a href="{{ route('auth.google.redirect') }}"
               class="btn login-portal-social-btn {{ $btnClass ?? '' }}"
               data-oauth-link
               data-oauth-base="{{ route('auth.google.redirect') }}"
        ...
        @endif
        @if ($facebookSignInEnabled ?? false)
            <a href="{{ route('auth.facebook.redirect') }}"
               class="btn login-portal-social-btn {{ $btnClass ?? '' }}"
               data-oauth-link
               data-oauth-base="{{ route('auth.facebook.redirect') }}"
